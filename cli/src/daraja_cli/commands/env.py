...
def list_environments(ctx: click.Context) -> None:
    ...
    if not endpoints:
        ...
        console.print(
            "[dim]Use 'daraja config set-endpoint' to add environments.[/dim]"
        )
        return
...
def switch(ctx: click.Context, environment: str) -> None:
    ...
    try:
        ...
        console.print(
            f"[dim]Webhooks will now be delivered to:[/dim] {endpoints[environment]}"
        )
...
def status(ctx: click.Context) -> None:
    ...
    if current_env not in endpoints:
        console.print(
            f"[red]❌ Current environment '{current_env}' has no configured endpoint[/red]"
        )
        return
...
def remove(ctx: click.Context, environment: str) -> None:
    ...
    if current_env == environment:
        if endpoints:
            ...
            console.print(
                f"[blue]ℹ️  Switched current environment to '{new_env}'[/blue]"
            )
        else:
