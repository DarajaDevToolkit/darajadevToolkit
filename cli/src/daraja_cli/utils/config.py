...
def load_profile(profile_name: Optional[str] = None) -> Dict[str, Any]:
    ...
    if keyring:
        ...
        raise ConfigError(
            "Secure storage (keyring) is not available. Please install 'keyring'."
        )
...
def delete_profile(profile_name: str) -> None:
    ...
    if keyring:
        try:
            keyring.delete_password("daraja-cli", profile_name)
        except Exception:
            # Keyring deletion might fail if password wasn't stored
            pass
    ...
    # If this was the current profile, clear current_profile
    if all_conf.get("current_profile") == profile_name:
        all_conf["current_profile"] = None
...
def rename_profile(old_name: str, new_name: str) -> None:
    ...
    if old_name not in profiles:
        raise ConfigError(f"Profile '{old_name}' not found.")
    ...
    # Move credentials in keyring
    if keyring:
        try:
            api_key = keyring.get_password("daraja-cli", old_name)
            if api_key:
                keyring.set_password("daraja-cli", new_name, api_key)
                keyring.delete_password("daraja-cli", old_name)
        except Exception:
            # Handle keyring errors gracefully
            pass

    # Update current profile if needed
    if all_conf.get("current_profile") == old_name:
        all_conf["current_profile"] = new_name
